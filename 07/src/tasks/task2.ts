...
const descendingSum = (num: number): number => {
  if (num === 0) {
    return num;
  }
  return num + descendingSum(num - 1);
}
...
const Task = async () => {
  ...
  const minFuelConsumption = {
    position: 0,
    fuelConsumption: Number.MAX_VALUE,
  };
  
  for (let targetPosition = 0; targetPosition <= maxDistance; targetPosition += 1) {
    let totalFuelConsumption = 0;
    positions.map((position) => {
      totalFuelConsumption += descendingSum(Math.abs(position - targetPosition));
    });
